...
export function createLogger(prod = false): winston.Logger {
  const logger = winston.createLogger({
    ...
    transports: [new winston.transports.File({ filename: `logs/error-${format(Date.now(), "dd-MM-yyyy-HH-mm-ss")}.log`, level: "error" }), new winston.transports.File({ filename: `logs/logs-${format(Date.now(), "dd-MM-yyyy-HH-mm-ss")}.log` })],
